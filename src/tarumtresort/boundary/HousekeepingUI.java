...
/**
 * Boundary class handling room cleaning logs and rollback tracking.
 * Demonstrates LIFO Stack integration.
 * 
 * @author Admin
 */
public class HousekeepingUI {
    ...
    private void updateStatus() {
        ...
        Room.HousekeepingStatus newStatus = Room.HousekeepingStatus.values()[statusChoice - 1];
    ...
    private void rollbackMultiple() {
        ...
        System.out.printf("\nSUCCESS: Popped %d items. The following updates have been undone:\n", rolledBack.getLength());
        for (int i = 1; i <= rolledBack.getLength(); i++) {
            System.out.println("  - " + rolledBack.getEntry(i));
        }
        InputUtil.pressEnterToContinue();
    }
    ...
    private void viewStack() {
        ...
        System.out.println("----------------------------------------------------------------------------------------------------");
        for (int i = 1; i <= logs.getLength(); i++) {
            HousekeepingLog l = logs.getEntry(i);
            System.out.printf("%-5d | %-10s | %-8d | %-15s | %-20s | %-20s\n",
                    i, l.getTimestamp(), l.getRoomNumber(), l.getSupervisorName(), l.getOldStatus().getLabel(), l.getNewStatus().getLabel());
        }
        System.out.println("----------------------------------------------------------------------------------------------------");
        InputUtil.pressEnterToContinue();
    }
    ...
    private void viewRoomsRegistry() {
        ...
        for (int i = 1; i <= rooms.getLength(); i++) {
            ...
            System.out.printf("%-8d | %-12s | %-22s | %-15s\n",
                    r.getRoomNumber(), r.getRoomType().getLabel(), r.getStatus().getLabel(), occupancy);
        }
        System.out.println("-----------------------------------------------------------------------------");
        InputUtil.pressEnterToContinue();
    }
